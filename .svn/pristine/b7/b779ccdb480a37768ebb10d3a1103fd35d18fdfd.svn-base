<!--
 * @Author: kevin
 * @Date: 2022-05-12 9:11:57
 * @LastEditors: kevin
 * @LastEditTime: 2022-05-19 13:38:41
 * @Description: 待办办理
-->
<template>
  <el-row :gutter="20">
  ...
  <div class="useTable" style="margin-top: 20px;">
    ...
    <div style="text-align: right; margin-top: 20px;">
      <!-- 非查看可以提交 -->
      <el-button @click="submitHandle" type="primary">提交</el-button>
...
<script setup>
  ...
  import { handleFile, formatTableData } from './config/hooks'
...
const getTargetData = async () => {
    const { data } = await getdetail({ detailId: props.rowData.detailId, type: props.rowData.type })
    showData.value = data.evaluateCheckTargetDto
    // console.log(props.rowData, 'props.rowData.idprops.rowData.id')
     const { tableData } = formatTableData(data.evaluateCheckTargetDto)

 ...
 * @LastEditors: kevin
 * @LastEditTime: 2022-04-29 17:00:34
 * @Description: 目标编辑
-->
<template>
  <el-row :gutter="20">
    ...
    <table style="width: 100%;">
      ...
      <template v-for="(item, index) in showTableData" :key="index">
        <tr>
          ...
          <td> <el-input v-model="item.selfScore" :disabled="role.userType === 1" @blur="count('ZP')" style="width: 70px;" /></td>
          <td v-if="role.userType === 1"> <el-input v-model="item.repeatedScore" :disabled="role.userType !== 1" @blur="count('FP')" style="width:70px;" /></td>
        </tr>
      </template>
    </table>

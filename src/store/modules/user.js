...
const user = {
  state: {
    ...
    avatar: '',
    buttons: [], // 按钮权限
    adminType: '', // 是否是超管 0超级管理员 1后台人员 2招聘单位 3招聘考生 4：COMPANY，单位； 5：COMMITTEE，委人事处
    roles: [],
    info: {}
  },
  ...
  actions: {
    ...
    Login ({ commit }, userInfo) {
      return new Promise((resolve, reject) => {
        const { isCollect } = userInfo
        login(userInfo, isCollect).then(response => {
          if (!response.success) {
    ...
    MenuChange ({ commit }, application) {
      return new Promise((resolve) => {
        sysMenuChange({ application: application.code }).then((res) => {
          ...
          console.log('all_app_menu', all_app_menu)
